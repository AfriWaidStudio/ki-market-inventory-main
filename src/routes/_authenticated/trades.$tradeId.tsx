...
function TradeDetailPage() {
  ...
  const [emotionalState, setEmotionalState] = useState<
    "fomo" | "anxious" | "confident" | "neutral" | "tilted"
  >("neutral");
  ...
  const saveJournal = useMutation({
    mutationFn: () =>
      journalSaveFn({
        data: { trade_id: tradeId, emotional_state: emotionalState, lessons_learned: lessons },
      }),
    onSuccess: (res) => {
  ...
  if (!q.data)
    return (
      <AppShell title="Trade Details">
        <div className="p-8 text-center text-rose-400 font-bold">Record not found.</div>
      </AppShell>
    );

  const { trade: t, notes, events, fees, ledger } = q.data;
  const p =
    t.realized_profit != null && Number(t.realized_profit) !== 0
      ? Number(t.realized_profit)
      : Number(t.actual_profit ?? t.expected_profit ?? 0);
  ...
  return (
    <AppShell title={`Trade · ${t.route ?? ""}`}>
      <div className="mb-6">
        <Link
          to="/trades"
          className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-cyan-400 transition-colors"
        >
        ...
        <div className="xl:col-span-2 space-y-6">
          <Card variant="glass" className="p-6 lg:p-8">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6 mb-6">
              <div>
                <h2 className="text-xl font-black text-white drop-shadow-md tracking-tight mb-2">
                  {t.route}
                </h2>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge
                    variant={
                      t.status === "active" ? "info" : t.status === "closed" ? "profit" : "default"
                    }
                  >
                    {t.status}
                  </Badge>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              ...
              <Info
                label="Remaining"
                value={`${fmtNumber(Number(t.remaining_amount ?? t.amount), 6)} ${t.asset}`}
              />
              <Info
                label="Stage"
                value={String(t.stage ?? t.status).replaceAll("_", " ")}
                className="capitalize"
              />
              <Info label="Buy" value={fmtMoney(Number(t.buy_price), t.currency)} />
              <Info
                label={isClosed ? "Actual sell" : "Expected sell"}
                value={fmtMoney(Number(t.actual_sell_price ?? t.expected_sell_price), t.currency)}
              />
              <Info
                label={isClosed ? "Realized P/L" : "Expected P/L"}
                value={fmtMoney(p, t.currency)}
                tone={p >= 0 ? "profit" : "loss"}
              />
              <Info
                label="Estimated fees"
                value={fmtMoney(Number(t.estimated_fees ?? 0), t.currency)}
              />
              <Info
                label="Recorded fees"
                value={fmtMoney(Number(t.total_recorded_fees ?? 0), t.currency)}
              />
              {t.final_fees != null && (
                <Info label="Final fees" value={fmtMoney(Number(t.final_fees), t.currency)} />
              )}
              <Info
                label="Confidence"
                value={`${fmtNumber(Number(t.confidence_score), 0)}%`}
                tone={Number(t.confidence_score) >= 70 ? "profit" : "warning"}
              />
              ...
              {t.sell_time && (
                <Info label="Closed" value={new Date(t.sell_time as string).toLocaleString()} />
              )}
              {t.duration_minutes != null && (
                <Info label="Duration" value={`${t.duration_minutes}m`} />
              )}
              {t.ki_accuracy_verdict && (
                <Info
                  label="KI verdict"
                  value={t.ki_accuracy_verdict}
                  tone={t.ki_accuracy_verdict === "accurate" ? "profit" : "warning"}
                />
              )}
            </div>
          </Card>
                  ...
                  <tbody className="font-medium">
                    {ledger.map((entry: any) => (
                      ...
                      >
                        ...
                        <td
                          className={`py-3 px-4 text-right tabular-nums ${Number(entry.amount) >= 0 ? "text-emerald-400 drop-shadow-glow-emerald" : "text-rose-400 drop-shadow-glow-rose"}`}
                        >
        ...
        <div className="space-y-6">
          ...
          <Card variant="glass" className="p-6">
            <div className="flex items-start justify-between mb-6">
              ...
              {journal.data && (
                <Badge variant={journal.data.ai_risk_score > 50 ? "warning" : "info"}>
            ...
            <div className="space-y-4">
              {journal.data ? (
                <div className="rounded-xl bg-black/40 border border-white/5 p-4 space-y-2">
                  ...
                  <Badge variant="info">{journal.data.emotional_state.toUpperCase()}</Badge>
                  <div className="mt-4 text-xs font-bold text-slate-500 uppercase tracking-widest">
                    Lessons Learned
                  </div>
                  <div className="text-sm font-medium text-slate-200">
                    {journal.data.lessons_learned}
                  </div>
                  ...
                  {journal.data.ai_risk_score > 50 && (
                    <div className="mt-4 p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg flex items-start gap-2">
                      ...
                      <div className="text-xs text-rose-300 font-medium leading-relaxed">
                        <strong>AI Risk Warning:</strong> You logged this trade under a high-risk
                        emotional state. The engine recommends taking a 24-hour break from manual
                        trading and switching to paper mode to avoid tilt.
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <Button
                    ...
                    disabled={!lessons.trim() || saveJournal.isPending}
                    className="w-full"
                  >
...
function Info({
  ...
  const cls =
    tone === "profit"
      ...
      : tone === "loss"
        ? "text-rose-400 drop-shadow-glow-rose"
        : tone === "warning"
