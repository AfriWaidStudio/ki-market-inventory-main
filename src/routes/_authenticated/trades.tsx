...
import { listTrades, updateTradePrice, markClosed, cancelTrade } from "@/lib/trades.functions";
...
function TradesPage() {
  ...
  const doClose = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        `${res.fully_closed ? "Closed" : "Partially closed"}. ${p >= 0 ? "Profit" : "Loss"}: ${fmtNumber(p)}`,
      );
    },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  ...
  return (
    <AppShell
      title="Active Trades"
      description="Manage open positions and track ongoing market performance."
      icon={Activity}
      headerActions={
        <Badge variant={rows.length > 0 ? "info" : "default"}>
        ...
        {rows.length === 0 ? (
          <div className="p-16 text-center">
            ...
            <p className="text-sm font-medium text-slate-400">
              No active trades. Open one from the{" "}
              <Link to="/scanner" className="text-cyan-400 font-bold hover:underline">
                Scanner
              </Link>
              .
            </p>
                  ...
                  <Fragment key={t.id}>
                    <tr className="border-t border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right text-slate-300">
                        {fmtNumber(Number(t.amount), 0)} {t.asset}
                      </td>
                      <td className="py-4 px-6 text-right text-slate-300">
                      ...
                      <td className="py-4 px-6 text-right">
                        {editing === t.id ? (
                          <div className="flex gap-2 justify-end items-center">
                            ...
                            <Button
                              onClick={() =>
                                doUpdate.mutate({ id: t.id, price: Number(priceInput) })
                              }
                          ...
                          <button
                            onClick={() => {
                              setEditing(t.id);
                              setPriceInput(String(t.expected_sell_price ?? ""));
                            }}
                      ...
                      <td
                        className={`py-4 px-6 text-right font-bold ${Number(t.expected_profit) >= 0 ? "text-emerald-400 drop-shadow-glow-emerald" : "text-rose-400 drop-shadow-glow-rose"}`}
                      >
                      ...
                      <td className="py-4 px-6 text-center">
                        <Badge
                          variant={
                            Number(t.confidence_score) >= 70
                              ? "profit"
                              : Number(t.confidence_score) >= 50
                                ? "info"
                                : "warning"
                          }
                        >
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex gap-2 justify-end">
                          <Button
                            ...
                            onClick={() => {
                              setClosingId(t.id);
                              setActualPrice(String(t.expected_sell_price ?? ""));
                              setFinalFees(String(t.estimated_fees ?? "0"));
                              setAmountSold(String(t.remaining_amount ?? t.amount));
                            }}
                            className="px-3 py-1.5 h-auto text-xs border-cyan-500/30 hover:border-cyan-500/60 text-cyan-400"
                          >
                            <CheckCircle2 className="w-3.5 h-3.5 mr-1" />{" "}
                            {t.trade_type === "paper" ? "Paper Close" : "Close"}
                          </Button>
                          <Button
                            variant="cyber-ghost"
                            onClick={() => doCancel.mutate(t.id)}
                    ...
                    {closingId === t.id && (
                      <tr className="bg-black/40 shadow-inner">
                        <td
                          colSpan={10}
                          className="px-6 py-5 border-t border-b border-cyan-500/20 relative"
                        >
                            ...
                            <Button
                              onClick={() =>
                                doClose.mutate({
                                  ...
                                  amountSold: Number(amountSold) || undefined,
                                })
                              }
