...
function AnalyticsPage() {
  ...
  if (q.isLoading)
    return (
      <AppShell title="Analytics">
        <div className="text-slate-400 font-medium animate-pulse">Loading analytics matrix…</div>
      </AppShell>
    );
  const d = q.data ?? {
    profitByDay: [],
  ...
  return (
    <AppShell
      ...
      description="Detailed statistical breakdown of your portfolio."
      icon={BarChart3}
    >
...
function ChartCard({
  ...
  return (
    <div
      className={`relative group overflow-hidden rounded-[2rem] bg-slate-900/50 backdrop-blur-xl p-6 border border-white/10 shadow-glass transition-all ${className}`}
    >
      <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
      <div className="relative z-10">
        <h3 className="text-sm font-black uppercase tracking-widest text-white drop-shadow-sm mb-6">
          {title}
        </h3>
        {children}
      </div>
