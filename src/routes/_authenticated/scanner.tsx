  ...
  return (
    <AppShell title="Opportunity Scanner">
      <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
        ...
        {/* LOG P2P PRICE CARD */}
        <div className="relative group overflow-hidden rounded-[2rem] bg-slate-900/50 backdrop-blur-xl p-8 border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_15px_35px_rgba(0,0,0,0.5)] h-fit">
          <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
          <div className="relative z-10 space-y-6">
            <div>
              <h2 className="text-sm font-black uppercase tracking-widest text-white drop-shadow-sm">Log a P2P price</h2>
              <p className="mt-1 text-xs font-medium text-slate-400">Manual entry. Add one buy and one sell per exchange to populate the scanner.</p>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="relative group overflow-hidden rounded-[2rem] bg-slate-900/50 backdrop-blur-xl p-8 border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_15px_35px_rgba(0,0,0,0.5)]">
            <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
            <div className="relative z-10">
              <div className="flex items-center justify-between gap-3 flex-wrap mb-6">
                <div>
                  <h2 className="text-sm font-black uppercase tracking-widest text-white drop-shadow-sm flex items-center gap-2">
                    <Zap className="h-4 w-4 text-cyan-400 drop-shadow-[0_0_8px_rgba(6,182,212,0.8)]" /> Live Opportunities
                  </h2>
                  <p className="text-xs font-medium text-slate-400 mt-1">
                    Auto-fetched every 2 min. Paper Buy simulates only; I Bought records external actions.
              ...
              {opps.isLoading ? (
                <div className="rounded-xl bg-black/40 border border-white/5 p-8 text-center shadow-inner">
                  <p className="text-sm font-medium text-slate-400 animate-pulse">Scanning the market for opportunities…</p>
                </div>
              ) : (opps.data ?? []).length === 0 ? (
                <div className="rounded-xl bg-black/40 border border-white/5 p-8 text-center shadow-inner">
                  <p className="text-sm font-medium text-slate-400">No opportunities detected yet. Awaiting fresh snapshots.</p>
                </div>
              ) : (
                    ...
                    <tbody className="tabular-nums font-medium">
                      {(opps.data ?? []).map((o, i) => (
                        <tr key={i} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                          <td className="py-4 px-6 text-white font-bold">{o.buy_exchange} <span className="text-slate-500 font-normal mx-1">→</span> {o.sell_exchange}</td>
                          <td className="text-right py-4 px-6 text-slate-300">{fmtMoney(o.buy_price, o.currency)}</td>
                          <td className="text-right py-4 px-6 text-slate-300">{fmtMoney(o.sell_price, o.currency)}</td>
                          <td className="text-right py-4 px-6 text-white">
                            {fmtNumber(o.spread)} <br/>
                            <span className="text-[10px] text-cyan-400 font-bold tracking-wider">({(o.spreadPct * 100).toFixed(2)}%)</span>
                          </td>
                          <td className={`text-right py-4 px-6 font-bold ${o.netProfit >= 0 ? "text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)]" : "text-rose-400 drop-shadow-[0_0_8px_rgba(251,113,133,0.3)]"}`}>
          ...
          <div className="relative group overflow-hidden rounded-[2rem] bg-slate-900/50 backdrop-blur-xl p-8 border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_15px_35px_rgba(0,0,0,0.5)]">
            <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
            <div className="relative z-10">
              <h2 className="text-sm font-black uppercase tracking-widest text-white drop-shadow-sm mb-6">Recent Snapshots</h2>
              ...
              <div className="rounded-xl bg-black/40 border border-white/5 p-4 shadow-inner">
                <div className="space-y-2 text-xs font-mono font-medium">
                  {(snaps.data ?? []).slice(0, 15).map((s: any) => (
                    <div key={s.id} className="flex justify-between items-center p-2 rounded-lg hover:bg-white/5 transition-colors">
                      <span className="text-slate-500">{new Date(s.captured_at as string).toLocaleTimeString()}</span>
                      <span className="text-slate-300 px-2 py-1 bg-white/5 rounded-md border border-white/5">{s.exchange} <span className={s.side === "buy" ? "text-emerald-400" : "text-rose-400"}>{s.side.toUpperCase()}</span></span>
                      <span className="text-white font-bold">{fmtMoney(Number(s.price), s.currency)}</span>
                    </div>
                  ))}
                  {(snaps.data ?? []).length === 0 && (
                    <div className="text-center p-4 text-slate-500">No network snapshots available in cache.</div>
                  )}
                </div>
              </div>
            </div>
...
const inputCls = "mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white shadow-inner focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all font-medium";
...
function RecBadge({ rec, confidence, risk }: { rec: string; confidence: number; risk: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-[10px] font-bold text-slate-500 tracking-widest">C{confidence.toFixed(0)} R{risk.toFixed(0)}</span>
    </div>
  );
}
