    ...
    <div className="space-y-4 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {inferences.map(inf => {
          ...
          return (
            <GlassCard key={inf.id} className="p-4 border-l-4 border-l-cyber-blue relative overflow-hidden group">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cyber-blue to-transparent opacity-50" />
              
              <div className="flex flex-col h-full justify-between">
                <div>
                  <p className="text-sm text-gray-400 mb-1">{new Date(inf.created_at).toLocaleTimeString()}</p>
