...
export async function buildKiGrounding(userId: string) {
  ...
  const [
    ...
    (db as any)
      ...
      .limit(100),
    (db as any).from("ki_position_plans").select("*").eq("user_id", userId).eq("active", true),
    (db as any)
      ...
      .limit(20),
    (db as any).from("market_intelligence_feed_health").select("*"),
    (db as any)
      .from("market_intelligence_candles")
      .select(
        "exchange,fiat,side,interval_seconds,bucket_at,close,executable_price,depth_asset,merchant_count,volatility",
      )
      .eq("interval_seconds", 300)
      .order("bucket_at", { ascending: false })
      .limit(240),
    (db as any)
      .from("ki_model_metrics")
      .select("*")
  ...
  return {
    ...
    open_and_recent_trades: trades ?? [],
    active_operator_plans: plans ?? [],
...
export async function answerKi(userId: string, text: string) {
  const key = process.env.LOVABLE_API_KEY;
  if (!key) throw new Error("Missing LOVABLE_API_KEY");
  const grounding = await buildKiGrounding(userId);
  const gateway = createLovableAiGatewayProvider(key);
  return (
    await generateText({
      model: gateway("google/gemini-2.5-flash"),
      system: buildKiSystem(grounding),
      prompt: text,
    })
  ).text;
}
