...
export function quoteExecutableSale(
  ads: MarketAd[],
  amount: number,
  now = Date.now(),
  ...
  const eligible = ads.filter(
    (a) =>
      ...
      a.price > 0 &&
      a.availableAsset > 0 &&
      now - new Date(a.observedAt).getTime() <= 120_000,
  );
  const byExchange = new Map<string, MarketAd[]>();
  for (const ad of eligible)
    byExchange.set(ad.exchange, [...(byExchange.get(ad.exchange) ?? []), ad]);
  ...
  for (const [exchange, rows] of byExchange) {
    ...
    for (const ad of rows) {
      ...
      if (
        (ad.minFiat && fiat < ad.minFiat) ||
        (ad.maxFiat && fiat > ad.maxFiat && ad.maxFiat / ad.price <= 0)
      )
        continue;
      const boundedFill = ad.maxFiat ? Math.min(fill, ad.maxFiat / ad.price) : fill;
      if (boundedFill <= 0) continue;
      proceeds += boundedFill * ad.price;
      remaining -= boundedFill;
      used += 1;
...
export function evaluatePosition(
  ...
  const quote = quoteExecutableSale(
    ads,
    position.remainingAmount - position.transferFeeAsset,
    now.getTime(),
  );
  ...
  const expectedNet = quote
    ? quote.proceeds -
      ((position.totalFiatSpent ?? position.remainingAmount * position.buyPrice) +
        position.entryFees +
        position.exitFeesFiat)
    : null;
  ...
  const conservativeUpside =
    recent.length >= 20
      ? [...recent].sort((a, b) => a - b)[Math.floor(recent.length * 0.65)]
      : breakEven;
  ...
  if (quote && history.feedHealthy) {
    ...
    else if (heldHours >= position.horizonHours && (regime === "falling" || reachable < 0.25))
      action = expectedNet != null && expectedNet >= 0 ? "sell_now" : "avoid";
    else action = "wait";
  }
  const evidence = [
    `All-in break-even is ${breakEven.toFixed(2)} per USDT.`,
    quote
      ? `${quote.exchange} can execute ${quote.filledAsset.toFixed(2)} USDT near ${quote.price.toFixed(2)}.`
      : "Executable depth is unavailable.",
    `Market regime is ${regime}; position has been held ${heldHours.toFixed(1)} hours.`,
    eligible
      ? `${(reachable * 100).toFixed(0)}% of recent observations met break-even.`
      : "Confidence is withheld until enough healthy observations exist.",
  ];
  const confidence = eligible
    ? Math.max(
        ...
        Math.min(
          95,
          35 + reachable * 50 + (regime === "rising" ? 10 : regime === "falling" ? -10 : 0),
        ),
      )
    : null;
  return {
    action,
    venue: quote?.exchange ?? null,
    executablePrice: quote?.price ?? null,
    executableAmount: quote?.filledAsset ?? 0,
    breakEvenPrice: breakEven,
    targetPrice: target,
    expectedNet,
    downside,
    targetWindowHours: eligible ? (reachable > 0.6 ? 1 : reachable > 0.35 ? 6 : 24) : null,
    confidence,
    confidenceEligible: eligible,
    regime,
    evidence,
    missingData: missing,
    invalidationCondition:
      "Recompute if executable price, depth, costs, trade stage, or feed health changes materially.",
    nextEvaluationAt: new Date(now.getTime() + 30_000).toISOString(),
  };
}

export function dedupeAlertKey(
  tradeId: string,
  type: string,
  action: string,
  price?: number | null,
