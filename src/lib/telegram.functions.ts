...
export const getTelegramStatus = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data } = await (context.supabase as any)
      ...
      .select("telegram_user_id,chat_id,linked_at,enabled")
...
export const createTelegramLink = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .handler(async ({ context }) => {
    const code = newOpaqueToken(9).toUpperCase();
    await (context.supabase as any).from("telegram_connections").upsert(
      {
        user_id: context.userId,
        link_code_hash: hashToken(code),
        link_code_expires_at: new Date(Date.now() + 600_000).toISOString(),
        enabled: true,
      },
      { onConflict: "user_id" },
    );
