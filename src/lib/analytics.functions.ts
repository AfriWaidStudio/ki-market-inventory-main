...
function isRealizedProfit(entry: LedgerRow): boolean {
  return (
    entry.entry_type === "paper_realized_profit" || entry.entry_type === "manual_realized_profit"
  );
}
  ...
  .handler(async ({ context }) => {
    const [
      ...
      (context.supabase as any)
        ...
        .eq("user_id", context.userId),
      (context.supabase as any)
        .from("market_inventory_exchange_balances")
        .select("*")
        .eq("user_id", context.userId),
    ]);
    ...
    const profitInWindow = (from: Date, tradeType?: "paper" | "manual") =>
      realizedLedger
        .filter(
          (entry) =>
            new Date(entry.created_at) >= from && (!tradeType || entry.trade_type === tradeType),
        )
    ...
    const wins = closed.filter(
      (trade) => num(trade.realized_profit ?? trade.actual_profit) > 0,
    ).length;
    ...
    const routeArr = Array.from(routeStats.entries()).map(([route, values]) => ({
      route,
      ...values,
    }));
    ...
    const trackedCapital = active
      ...
      .reduce(
        (sum, trade) => sum + num(trade.remaining_amount ?? trade.amount) * num(trade.buy_price),
        0,
      );
    ...
    const recentWithdrawals = txs.filter(
      (t: any) => t.type === "withdrawal" && new Date(t.tx_time) >= oneHourAgo,
    );
    const recentDeposits = txs.filter(
      (t: any) => t.type === "deposit" && new Date(t.tx_time) >= oneHourAgo,
    );

    for (const w of recentWithdrawals) {
      // Basic check: is there a deposit of similar amount (+/- 5%) after this withdrawal?
      const match = recentDeposits.find(
        (d: any) =>
...
export const analyticsSeries = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const byType = new Map<
      string,
      { date: string; paper: number; manual: number; total: number }
    >();
    ...
    let running = 0;
    const capitalGrowth = profitByDay.map((day) => ({
      date: day.date,
      cumulative: (running += day.profit),
    }));
