  ...
  const value = useMemo<ContextValue>(
    () => ({
      ...
      signUp: async (email, password, displayName) => {
        ...
        setError(null);
        try {
          const result = await register({ data: { email, password, displayName } });
          if (result.error) {
            setError(result.error);
            return result;
          }
...
export function useUser() {
  ...
  return { user, loading, displayName: user?.displayName ?? user?.email ?? null };
}
...
export function useOAuth() {
  ...
  return { signInWithOAuth, loading };
}
