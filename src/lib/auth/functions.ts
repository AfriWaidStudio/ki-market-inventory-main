...
export const getCurrentUser = createServerFn({ method: "GET" }).handler(
  async () => (await getCurrentSession())?.user ?? null,
);

export const register = createServerFn({ method: "POST" })
  .inputValidator((value: unknown) =>
    credentials.extend({ displayName: z.string().trim().max(80).optional() }).parse(value),
  )
  .handler(async ({ data }) => {
    assertSameOrigin();
    ...
    await (supabaseAdmin as any).from("auth_identities").insert({
      user_id: user.id,
      provider: "password",
    ...
    await (supabaseAdmin as any)
      .from("profiles")
      .upsert({ user_id: user.id, display_name: data.displayName || email });
...
export const login = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: user } = await (supabaseAdmin as any)
      ...
      .eq("email", normalizeEmail(data.email))
      .maybeSingle();
    if (
      !user?.password_hash ||
      user.status !== "active" ||
...
export const requestPasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (process.env.NODE_ENV === "production")
      throw new Error("Password email transport is not configured.");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: user } = await (supabaseAdmin as any)
      .from("app_users")
      .select("id")
    ...
    if (user) {
      ...
      await (supabaseAdmin as any).from("password_reset_tokens").insert({
        user_id: user.id,
        token_hash: hashToken(token),
        expires_at: new Date(Date.now() + 30 * 60 * 1000).toISOString(),
      });
      const baseUrl = new URL((await import("@tanstack/react-start/server")).getRequest().url)
        .origin;
      console.info(
        `[Auth] Password reset link for ${normalizeEmail(data.email)}: ${baseUrl}/auth?reset=${encodeURIComponent(token)}`,
      );
    }
    return { ok: true };
  });
...
export const resetPassword = createServerFn({ method: "POST" })
  .validator((value: unknown) =>
    z.object({ token: z.string().min(20), password: z.string().min(10).max(128) }).parse(value),
  )
  .handler(async ({ data }) => {
    assertSameOrigin();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const now = new Date().toISOString();
    const { data: reset } = await (supabaseAdmin as any)
      .from("password_reset_tokens")
      .select("id,user_id,expires_at,consumed_at")
      .eq("token_hash", hashToken(data.token))
      .maybeSingle();
    if (!reset || reset.consumed_at || reset.expires_at <= now)
    ...
    await (supabaseAdmin as any)
      ...
      .upsert(
        { user_id: reset.user_id, provider: "password", provider_subject: reset.user_id },
...
export const beginGoogleOAuth = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const verifier = newOpaqueToken(48);
  const options = {
    httpOnly: true,
  ...
  setCookie("oauth_state", state, options);
  setCookie("oauth_nonce", nonce, options);
  setCookie("oauth_verifier", verifier, options);
