...
const interval = Math.max(15_000, Number(process.env.OPERATOR_POLL_MS ?? 30_000));
...
async function acquireLease() {
  ...
  const { data } = await db
    .from("ki_worker_leases")
    .select("owner_id,lease_until")
    .eq("lease_key", "market-operator")
    .maybeSingle();
  if (data && data.owner_id !== owner && new Date(data.lease_until).getTime() > Date.now())
    return false;
  await db.from("ki_worker_leases").upsert({
    lease_key: "market-operator",
    owner_id: owner,
    lease_until: until,
  ...
  for (const fiat of SUPPORTED_FIATS)
    for (const adapter of EXCHANGES)
      for (const side of ["buy", "sell"] as const) {
        const healthKey = `${adapter.name}:${fiat}`;
        const count = failures.get(healthKey) ?? 0;
        if (count > 0 && Date.now() % Math.min(300_000, 30_000 * 2 ** count) > interval) continue;
        try {
          const ads = await adapter.fetch("USDT", fiat, side);
          captured.push(...ads);
          failures.set(healthKey, 0);
          await db.from("market_intelligence_feed_health").upsert({
            exchange: adapter.name,
            fiat,
            status: ads.length ? "healthy" : "unsupported",
            schema_version: ads[0]?.schemaVersion,
            last_success_at: new Date().toISOString(),
            consecutive_failures: 0,
            latency_ms: ads[0]?.latencyMs,
            updated_at: new Date().toISOString(),
          });
        } catch (error) {
          const next = count + 1;
          failures.set(healthKey, next);
          await db.from("market_intelligence_feed_health").upsert({
            exchange: adapter.name,
            fiat,
            ...
            next_attempt_at: new Date(
              Date.now() + Math.min(300_000, 30_000 * 2 ** next),
            ).toISOString(),
            updated_at: new Date().toISOString(),
          });
        }
  ...
  if (captured.length)
    await db.from("market_intelligence_ads").insert(
      captured.map((a) => ({
        ...
        fiat: a.fiat,
        side: a.side,
        external_ad_id: a.externalAdId,
        price: a.price,
        available_asset: a.availableAsset,
        min_fiat: a.minFiat,
        max_fiat: a.maxFiat,
        payment_methods: a.paymentMethods,
        merchant_name: a.merchantName,
        merchant_verified: a.merchantVerified,
        completion_rate: a.completionRate,
        completed_orders: a.completedOrders,
        response_latency_ms: a.latencyMs,
        schema_version: a.schemaVersion,
        observed_at: a.observedAt,
        raw_fingerprint: a.rawFingerprint,
      })),
    );
  return captured;
}
      ...
      await db.from("market_intelligence_candles").upsert(
        {
          ...
          volatility:
            Math.sqrt(prices.reduce((s, p) => s + (p - mean) ** 2, 0) / prices.length) / mean,
        },
...
async function evaluatePositions(captured: CapturedAd[]) {
  ...
  for (const trade of trades ?? []) {
    ...
    const { data: health } = await db
      .from("market_intelligence_feed_health")
      .select("status")
      .eq("fiat", fiat);
    ...
    const { data: plan } = await db
      ...
      .insert({
        ...
        ...{
          ...
          executable_price: decision.executablePrice,
          executable_amount: decision.executableAmount,
          break_even_price: decision.breakEvenPrice,
          target_price: decision.targetPrice,
          expected_net: decision.expectedNet,
          downside: decision.downside,
          target_window_hours: decision.targetWindowHours,
          confidence: decision.confidence,
          confidence_eligible: decision.confidenceEligible,
    ...
    if (decision.action === "sell_now") {
      ...
      const { data: alert } = await db
        ...
        .upsert(
          {
            ...
            message: `${decision.venue} can currently execute near ${decision.executablePrice?.toFixed(2)} ${fiat}. Estimated net: ${decision.expectedNet?.toFixed(2)} ${fiat}.`,
            evidence: { decision },
          },
        ...
        .single();
      if (alert)
        await db.from("ki_alert_deliveries").upsert(
          [
            { alert_id: alert.id, channel: "in_app" },
...
async function dispatchTelegram() {
  ...
  const { data: deliveries } = await db
    .from("ki_alert_deliveries")
    .select("id,attempts,alert_id,ki_operator_alerts(user_id,message)")
    .eq("channel", "telegram")
    .in("status", ["pending", "failed"])
    .lte("next_attempt_at", new Date().toISOString())
    .limit(50);
  for (const d of deliveries ?? []) {
    const alert = Array.isArray(d.ki_operator_alerts)
      ? d.ki_operator_alerts[0]
      : d.ki_operator_alerts;
    if (!alert) continue;
    const [{ data: settings }, { data: connection }] = await Promise.all([
      db
        .from("ki_strategy_settings")
        .select("shadow_mode,live_alerts_enabled,muted_until")
    ...
    if (
      ...
      !settings.live_alerts_enabled ||
      !connection?.enabled ||
      !connection.chat_id ||
      new Date(settings.muted_until ?? 0).getTime() > Date.now()
    )
      continue;
    try {
      const r = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: connection.chat_id, text: alert.message }),
      });
...
async function run() {
  console.info(`[Operator] ${owner} started; interval ${interval}ms`);
  for (;;) {
    const start = Date.now();
    try {
      if (await acquireLease()) {
        const ads = await captureMarket();
        await aggregate(ads);
        await evaluatePositions(ads);
        await dispatchTelegram();
      }
    } catch (e) {
      console.error("[Operator] cycle failed", e);
    }
    await sleep(Math.max(1000, interval - (Date.now() - start)) + Math.floor(Math.random() * 3000));
  }
}
void run();
