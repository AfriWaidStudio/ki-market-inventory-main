...
async function runTest() {
  ...
  const { error } = await supabase.from('market_inventory_inferences').insert({
    ...
    confidence: 0.99,
    status: 'pending',
    context_data: { crypto_amount: 1200, crypto_asset: "USDT", fiat_amount: 1950000, fiat_asset: "NGN" }
  });

  if (error) {
    console.error("Failed to insert inference:", error);
  } else {
    console.log("Mock inference inserted successfully! Check your dashboard, it should appear via Realtime.");
  }
}
